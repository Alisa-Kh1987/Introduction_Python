...
#  Начало - общее для обоих способов решения: генерим случайную матрицу (условно размером 4х4)
...
# (коммент для себя): np.matrix - для вывода не в одну строку, а в 2D-формвте


def transp_matrica(my_matrix):
    my_matrix_transp = np.matrix([[my_matrix[j][i] for j in range(
        len(my_matrix))] for i in range(len(my_matrix[0]))])
    return my_matrix_transp
